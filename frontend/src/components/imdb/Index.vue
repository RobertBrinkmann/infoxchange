        ...
        <div class="container">
                ...
                <div class="col" v-for="movie in movies" v-bind:key="movie">
                    <router-link :to="{name: 'movie', params: {id: movie.imdbID, keywords: search}}" tag="div" class="card shadow-sm card-movie d-flex flex-column" @click="selectMovie(movie.imdbID)">
                        <div class="card-image bg-dark">
                            ...
                            <div class="image-error text-light text-center" v-if="movie.imageError">Sorry, couldn't find the image</div>
                        </div>
            ...
            <nav class="p-5" aria-label="Movie results navigation">
                <ul class="pagination justify-content-center" v-if="totalResults > 10">
                    <li class="page-item" v-bind:class="{disabled: page == 1}"><button type="button" class="page-link" @click="searchMovies(keywords, page - 1)"><font-awesome-icon icon="angle-left"/></button></li>
                ...
                <div class="text-center text-muted" v-if="search">{{totalResults}} results found ({{totalPages}} pages)</div>
            </nav>
        </div>
    </div>
</template>
